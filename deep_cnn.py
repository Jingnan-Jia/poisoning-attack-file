...
def savi_hotfig(img, img_path):
    ...
    sns.heatmap(img, annot=True, cmap='jet', fmt='.1f', vmax=0.1, vmin=-0.1,)
...
def get_gradient_of_x0(x, ckpt_path, number, grad_label, return_logits = True, new=True):
    ...
    inputs:
        ...
        ckpt_path: where the model is saved.
    ...
    with tf.Session() as sess:
        saver.restore(sess, ckpt_path)  # restore model weights
        ...
        print('output_probilities:',output_val[0])
        print('output_predicted_labels', np.argmax(output_val[0]))
        
        minus = []
        plus = []
        for t in range(FLAGS.nb_labels):
            ...
            for i in range(grads_mat_plus.shape[0]):
                for j in range(grads_mat_plus.shape[1]):
                    for k in range(grads_mat_plus.shape[2]):
                        if grads_mat_plus[i,j,k] < 0:
                            grads_mat_plus[i,j,k] = 0
            grads_mat_plus = grads_mat_plus/grads_mat_max  # rescale to [0~1]
            
            grads_mat_minus = copy.deepcopy(grads_mat)
            for i in range(grads_mat_minus.shape[0]):
                for j in range(grads_mat_minus.shape[1]):
                    for k in range(grads_mat_minus.shape[2]):
                        if grads_mat_minus[i,j,k] > 0:
                            grads_mat_minus[i,j,k] = 0
            grads_mat_minus = np.abs(grads_mat_minus/grads_mat_min)  # rescale to [0~1]
                        
            #print('--------',grads_mat_plus)

  
            if new==True:
                grads_path = '../image_save/'+str(FLAGS.dataset)+'_grads_number'+str(number)+'_class_'+str(t)+'_new.png'
                grads_plus_path = '../image_save/'+str(FLAGS.dataset)+'_grads_number'+str(number)+'_class_'+str(t)+'_new_plus.png'
                grads_minus_path = '../image_save/'+str(FLAGS.dataset)+'_grads_number'+str(number)+'_class_'+str(t)+'_new_minus.png'
                grads_hot_path = '../image_save/'+str(FLAGS.dataset)+'_grads_number'+str(number)+'_class_'+str(t)+'_hot_new.png'
...
# =============================================================================
                ...
                grads_path = '../image_save/'+str(FLAGS.dataset)+'_grads_number'+str(number)+'_class_'+str(t)+'.png'
                grads_plus_path = '../image_save/'+str(FLAGS.dataset)+'_grads_number'+str(number)+'_class_'+str(t)+'plus.png'
                grads_minus_path = '../image_save/'+str(FLAGS.dataset)+'_grads_number'+str(number)+'_class_'+str(t)+'minus.png'
                grads_hot_path = '../image_save/'+str(FLAGS.dataset)+'_grads_number'+str(number)+'_class_'+str(t)+'_hot.png'
...
# =============================================================================
        ...
        print('the label of the current data is: ', grad_label)
